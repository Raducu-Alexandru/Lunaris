...
export class ClassHomePageComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (this._socketConnectionService.getSocketState()) {
      ...
      this.socketConnectionSubscription =
        this._socketConnectionService.onSocketChangeStateSubject.subscribe({
  ...
  async onSendMessage(event): Promise<void> {
    ...
    if (messageResponse.data.succ) {
      this.newMessage = '';
      var newMessage: ClassMessageDetails =
        messageResponse.data.data.socketSendObject;
  ...
  private async _getClassDetails(): Promise<void> {
    var responseObject: ResponseObject =
      await this._authenticateService.sendGetReq(
        environment.classesUrl + `/get/class/${this.classId}/details`
      );
  ...
  private async _getClassMessageDetails(): Promise<void> {
    var responseObject: ResponseObject =
      await this._authenticateService.sendGetReq(
        environment.messagesUrl + `/get/class/${this.classId}/messages`
      );
    if (!this._authenticateService.checkResponse(responseObject)) {
      return null;
    }
