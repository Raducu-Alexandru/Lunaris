...
export class ClassesPageComponent implements OnInit {
  ...
  async onStudyYearSelectInput(studyYearId: number): Promise<void> {
    await this._getClassesPreviewDetails(studyYearId);
  }
  ...
  private async _getClassesPreviewDetails(studyYearId: number): Promise<void> {
    var responseObject: ResponseObject = await this._authenticateService.sendGetReq(environment.classesUrl + `/get/classes/${studyYearId}`);
