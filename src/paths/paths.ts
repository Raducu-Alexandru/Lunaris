...
import {
	AccountMethods,
	handleCheckProfessorMiddleware,
...
export function appRoutes(
	...
	router.get('/get/class/:classId/final-grades', async (req: Request, res: Response) => {
...
WHERE classes.classId = ? AND subjects.universityId = ?`,
	...
	router.post(
		'/add/members',
		handleCheckProfessorMiddleware(dbConnection, loginMethodsObj, accountMethodsObj),
...
WHERE classes.classId = ? AND studyYears.universityId = ?`,
			...
			try {
				...
				var insertClassMembersSql: string =
					'INSERT INTO classesMembers (classId, userId) VALUES (?, ?)';
				for (var i = 0; i < usersIds.length; i++) {
					checkUserIdSqlResult = await dbConnection.executeInTransaction<SelectPacket>(
						transaction,
						'SELECT userId FROM users WHERE userId = ? AND universityId = ?',
						[usersIds[i], universityId]
					);
					...
					checkUserIdClassMembersSqlResult = await dbConnection.executeInTransaction<SelectPacket>(
						transaction,
						'SELECT userId FROM classesMembers WHERE userId = ?',
						[usersIds[i]]
					);
					...
					await dbConnection.executeInTransaction<NormalPacket>(
						transaction,
						insertClassMembersSql,
						[classId, usersIds[i]]
					);
				}
	...
	router.post(
		...
		async (req: Request, res: Response) => {
			...
			var universityId: number = await accountMethodsObj.getUserUniveristy(userId);
			var userRole: number = await accountMethodsObj.getUserRole(userId);
			var checkClassMemberIdAndClassIdSqlResult: SelectPacket =
				await dbConnection.execute<SelectPacket>(
...
WHERE classesMembers.userId = ? AND classesMembers.classId = ? AND studyYears.universityId = ?`,
			...
			await dbConnection.execute<NormalPacket>(
				'DELETE FROM classesMembers WHERE userId = ? AND classId = ?',
				[memberUserId, classId]
			);
			responseObject = {
				succ: true,
			};
...
WHERE classes.classId = ? AND studyYears.universityId = ?`,
	...
	router.post(
		...
		async (req: Request, res: Response) => {
			...
			var usersIds: number[] = body.usersIds;
			var checkStudyYearIdSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>(
				'SELECT studyYearId FROM studyYears WHERE universityId = ? AND studyYearId = ?',
...
WHERE subjects.universityId = ? AND yearsSubjects.yearSubjectId = ?`,
			...
			var transaction = await dbConnection.startTransaction();
			try {
				var classInserSqlResult: NormalPacket =
					await dbConnection.executeInTransaction<NormalPacket>(
						transaction,
				...
				var insertClassMembersSql: string =
					'INSERT INTO classesMembers (classId, userId) VALUES (?, ?)';
				await dbConnection.executeInTransaction<NormalPacket>(transaction, insertClassMembersSql, [
					classInserSqlResult.insertId,
					userId,
				]);
				for (var i = 0; i < usersIds.length; i++) {
					checkUserIdSqlResult = await dbConnection.executeInTransaction<SelectPacket>(
					...
					await dbConnection.executeInTransaction<NormalPacket>(
						transaction,
						insertClassMembersSql,
						[classInserSqlResult.insertId, usersIds[i]]
					);
				}
				await dbConnection.commitTransactionAndClose(transaction);
			} catch (err) {
	...
	router.post(
		...
		async (req: Request, res: Response) => {
			...
			var body: CurrentBody = req.body;
			var responseObject: CustomResponseObject;
			if (!('email' in body && 'yearSubjectId' in body && 'studyYearId' in body)) {
				responseObject = {
					succ: false,
					mes: 'Something went wrong',
					debugMes: 'Invalid number of POST parameters',
			...
			var yearSubjectId: number = body.yearSubjectId;
			var studyYearId: number = body.studyYearId;
			var getUserIdFromEmailSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>(
				'SELECT userId, role, disabled FROM users WHERE email = ? AND universityId = ?',
				[email, universityId]
			);
...
WHERE studentsYears.userId = ? AND studentsYears.studyYearId = ? AND yearsSubjects.yearSubjectId = ?`,
	...
	router.get(
		'/get/class-subjects/:isArchived?',
		handleCheckProfessorMiddleware(dbConnection, loginMethodsObj, accountMethodsObj),
		async (req: Request, res: Response) => {
			var isArchived: boolean =
				parseInt(req.params.isArchived) == 1
					? true
					: parseInt(req.params.isArchived) == 0
					? false
					: null;
			var sessionInterfaceObj: SessionInterface = req['sessionInterfaceObj'];
			var loginMethodsInterfaceObj: LoginMethodsInterface = new LoginMethodsInterface(
...
ORDER BY programs.createdDate, years.yearIndex, yearsSubjects.semesterIndex ASC`,
	...
	router.get('/get/classes/student-year-id/:studentYearId', async (req: Request, res: Response) => {
		...
		var userRole: number = await accountMethodsObj.getUserRole(userId);
		var responseObject: CustomResponseObject;
		var getYearIdAndStudyYearIdSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>(
			'SELECT yearId, studyYearId FROM studentsYears WHERE studentYearId = ?',
			[studentYearId]
		);
...
WHERE classes.studyYearId = ? AND yearsSubjects.yearId = ? AND subjects.universityId = ?`,
				[
					getYearIdAndStudyYearIdSqlResult[0].studyYearId,
					getYearIdAndStudyYearIdSqlResult[0].yearId,
					universityId,
				]
			);
		} else {
...
WHERE classes.studyYearId = ? AND yearsSubjects.yearId = ? AND subjects.universityId = ? AND classesMembers.userId = ?`,
		...
		for (var i = 0; i < classesSqlResult.length; i++) {
			if (classesSqlResult[i].subjectName) {
				className =
					classesSqlResult[i].classPrefix +
					' ' +
					classesSqlResult[i].subjectName +
					' ' +
					classesSqlResult[i].classSuffix;
			} else {
