...
import { CustomResponseObject, ProgamPreviewDetails, ProgramEditDetails, ProgramYearEditDetails, SchoolEditDetails, SchoolPreviewDetails, StudentProgramPreviewDetails, StudentYearsDetails, StudyYearDetails, StudyYearStudentDetails, SubjectEditDetails, SubjectPreviewDetails, SubjectScholarSituation, UserEditDetails, UserTableDetails, YearEditDetails, YearPreviewDetails } from '@raducualexandrumircea/lunaris-interfaces';
...
export function appRoutes(router: Router, dbConnection: DbHandler, loginMethodsObj: LoginMethods, accountMethodsObj: AccountMethods, upload: multer.Multer) {
  ...
  router.post('/create/study-year', async (req: Request, res: Response) => {
    ...
    try {
      var studyYearNormalSqlResult: NormalPacket = await dbConnection.executeInTransaction<NormalPacket>(transaction, 'INSERT INTO studyYears (universityId, fromYear, toYear) VALUES (?, ?, ?)', [universityId, fromYear, toYear]);
...
WHERE users.universityId = ? AND users.role = 1 AND users.disabled = ?`;
      ...
      for (var i = 0; i < studentsSqlResult.length; i++) {
        nextYearSqlResult = await dbConnection.executeInTransaction<SelectPacket>(transaction, 'SELECT years.yearId FROM years WHERE years.programId = ? AND years.yearIndex = ?', [studentsSqlResult[i].programId, studentsSqlResult[i].maxYearIndex + 1]);
        if (nextYearSqlResult.length != 1) {
          continue;
        }
        await dbConnection.executeInTransaction<NormalPacket>(transaction, 'INSERT INTO studentsYears (yearId, userId, studyYearId) VALUES (?, ?, ?)', [nextYearSqlResult[0].yearId, studentsSqlResult[i].userId, studyYearNormalSqlResult.insertId]);
      }
  ...
  router.post('/create/student', async (req: Request, res: Response) => {
    ...
    var getLastStudyYearIdSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>('SELECT studyYearId FROM studyYears WHERE universityId = ? ORDER BY createdDate DESC LIMIT 1', [universityId]);
    ...
    try {
      ...
      var insertUserNormalPacket = await dbConnection.executeInTransaction<NormalPacket>(transaction, 'INSERT INTO users (universityId, email, hashedPassword, role, firstName, lastName) VALUES (?, ?, ?, ?, ?, ?)', [universityId, email, hashedPassword, 1, firstName, lastName]);
      var checkYearIdSqlResult: SelectPacket;
      for (var i = 0; i < yearIds.length; i++) {
        checkYearIdSqlResult = await dbConnection.executeInTransaction<SelectPacket>(transaction, `SELECT years.yearId FROM years
INNER JOIN programs ON years.programId = programs.programId
INNER JOIN schools ON programs.schoolId = schools.schoolId
WHERE years.yearId = ? AND schools.universityId = ?`, [yearIds[i], universityId]);
        ...
        await dbConnection.executeInTransaction<NormalPacket>(transaction, 'INSERT INTO studentsYears (yearId, userId, studyYearId) VALUES (?, ?, ?)', [yearIds[i], insertUserNormalPacket.insertId, getLastStudyYearIdSqlResult[0].studyYearId]);
      }
      await dbConnection.commitTransactionAndClose(transaction);
    } catch (err) {
      await dbConnection.rollbackTransactionAndClose(transaction);
  ...
  router.post('/update/scholar-situation', async (req: Request, res: Response) => {
    ...
    var checkStudentYearIdSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>(`SELECT studentsYears.userId FROM studentsYears
INNER JOIN users ON users.userId = studentsYears.userId
WHERE users.universityId = ? AND studentsYears.studentYearId = ?`, [universityId, studentYearId]);
    ...
    if (isInserting) {
      ...
      await dbConnection.execute<NormalPacket>('INSERT INTO finalGrades (studentYearId, subjectId, grade, credits) VALUES (?, ?, ?, ?)', [studentYearId, subjectId, grade, getSubjectCreditsSqlResult[0].credits]);
    } else {
  ...
  router.get('/get/scholar-situation/:studentYearId', async (req: Request, res: Response) => {
    ...
    var userId: number = await loginMethodsInterfaceObj.getLoggedInUserId();
    var universityId: number = await accountMethodsObj.getUserUniveristy(userId);
    var scholarSituationSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>(`SELECT yearsSubjects.subjectId, finalGrades.grade, finalGrades.credits as finalGradesCredits, subjects.credits as subjectsCredits, subjects.subjectName, yearsSubjects.semesterIndex FROM yearsSubjects
...
ORDER BY yearsSubjects.semesterIndex ASC`, [universityId, studentYearId]);
...
ORDER BY studyYears.createdDate DESC`, [universityId, studentUserId]);
  ...
  router.post('/create/student/program', async (req: Request, res: Response) => {
    ...
    var checkYearIdSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>(`SELECT years.yearId FROM years
INNER JOIN programs ON years.programId = programs.programId
INNER JOIN schools ON programs.schoolId = schools.schoolId
WHERE years.yearId = ? AND schools.universityId = ?`, [yearId, universityId]);
    if (checkYearIdSqlResult.length != 1) {
      responseObject = {
        succ: false,
        mes: 'You don not permissions to use this year as is not under your university',
    ...
    var checkStudentUserIdUniversitySqlResult: SelectPacket = await dbConnection.execute<SelectPacket>('SELECT userId FROM users WHERE userId = ? AND universityId = ? AND role = 1', [studentUserId, universityId]);
  ...
  router.get('/get/student/:studentUserId/programs', async (req: Request, res: Response) => {
    ...
    var userId: number = await loginMethodsInterfaceObj.getLoggedInUserId();
    var universityId: number = await accountMethodsObj.getUserUniveristy(userId);
    var studentProgramsSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>(`SELECT programs.programId, programs.programName, years.yearIndex FROM studentsYears
INNER JOIN years ON studentsYears.yearId = years.yearId
INNER JOIN programs ON years.programId = programs.programId
INNER JOIN users ON studentsYears.userId = users.userId
WHERE studentsYears.userId = ? AND users.universityId = ?`, [studentUserId, universityId]);
  ...
  router.get('/get/student/:studentUserId/platform-info', async (req: Request, res: Response) => {
    ...
    var userId: number = await loginMethodsInterfaceObj.getLoggedInUserId();
    var universityId: number = await accountMethodsObj.getUserUniveristy(userId);
    var studentSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>("SELECT email, firstName, lastName, disabled FROM users WHERE universityId = ? AND role = 1 AND userId = ?", [universityId, studentUserId]);
  ...
  router.get('/get/students', async (req: Request, res: Response) => {
    ...
    var userId: number = await loginMethodsInterfaceObj.getLoggedInUserId();
    var universityId: number = await accountMethodsObj.getUserUniveristy(userId);
    var studentsSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>("SELECT userId, email, CONCAT_WS(' ', firstName, lastName) as fullName FROM users WHERE universityId = ? AND role = 1", [universityId]);
    ...
    try {
      ...
      for (var yearSubject of year) {
        if (yearSubject.semesterIndex && yearSubject.subjectId) {
          ...
          await dbConnection.executeInTransaction<NormalPacket>(transaction, 'UPDATE yearsSubjects SET semesterIndex = ? WHERE subjectId = ? AND yearId = ?', [yearSubject.semesterIndex, yearSubject.subjectId, yearId]);
        }
      }
      await dbConnection.commitTransactionAndClose(transaction);
    } catch (err) {
...
WHERE years.yearId = ? AND schools.universityId = ?`, [yearId, universityId]);
    ...
    var yearSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>(`SELECT yearsSubjects.subjectId, yearsSubjects.semesterIndex, subjects.subjectName FROM yearsSubjects
INNER JOIN subjects ON yearsSubjects.subjectId = subjects.subjectId
WHERE yearsSubjects.yearId = ?`, [yearId]);
  ...
  router.get('/get/years/:programId', async (req: Request, res: Response) => {
    ...
    var userId: number = await loginMethodsInterfaceObj.getLoggedInUserId();
    var universityId: number = await accountMethodsObj.getUserUniveristy(userId);
    var checkProgramIdSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>(`SELECT programs.programId FROM programs
INNER JOIN schools ON programs.schoolId = schools.schoolId
WHERE programs.programId = ? AND schools.universityId = ?`, [programId, universityId]);
    ...
    var yearsSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>(`SELECT years.yearIndex, years.yearId, COUNT(yearsSubjects.yearId) AS noSubjects FROM years
INNER JOIN yearsSubjects ON yearsSubjects.yearId = years.yearId
WHERE years.programId = ?
GROUP BY years.yearId`, [programId]);
...
WHERE programs.programId = ? AND schools.universityId = ?`, [programId, universityId]);
    ...
    if (years.length > 0) {
      ...
      try {
        ...
        for (var i = 0; i < years.length; i++) {
          ...
          yearNormalSqlResult = await dbConnection.executeInTransaction<NormalPacket>(transaction, 'INSERT INTO years (programId, yearIndex) VALUES (?, ?)', [programId, lastYearIndexSqlResult[0].yearIndex + i + 1]);
          if (years[i].length == 0) {
            await dbConnection.rollbackTransactionAndClose(transaction);
            responseObject = {
              succ: false,
          ...
          for (var subject of years[i]) {
            ...
            await dbConnection.executeInTransaction<NormalPacket>(transaction, 'INSERT INTO yearsSubjects (yearId, subjectId, semesterIndex) VALUES (?, ?, ?)', [yearNormalSqlResult.insertId, subject.subjectId, subject.semesterIndex]);
          }
        }
        await dbConnection.commitTransactionAndClose(transaction);
      } catch (err) {
  ...
  router.get('/get/program/:programId', async (req: Request, res: Response) => {
    ...
    var userId: number = await loginMethodsInterfaceObj.getLoggedInUserId();
    var universityId: number = await accountMethodsObj.getUserUniveristy(userId);
    var programSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>(`SELECT programs.schoolId, programs.programName, programs.programType, programs.programShortName, programs.archived FROM programs
INNER JOIN schools ON programs.schoolId = schools.schoolId
WHERE programs.programId = ? AND schools.universityId = ?`, [programId, universityId]);
  ...
  router.get('/get/programs/:schoolId?', async (req: Request, res: Response) => {
    ...
    if (schoolId) {
      programsSqlResult = await dbConnection.execute<SelectPacket>(`SELECT programId, programName, programType, archived FROM programs
INNER JOIN schools ON programs.schoolId = schools.schoolId
WHERE schools.universityId = ? AND programs.schoolId = ?`, [universityId, schoolId]);
    } else {
      programsSqlResult = await dbConnection.execute<SelectPacket>(`SELECT programId, programName, programType, archived FROM programs
INNER JOIN schools ON programs.schoolId = schools.schoolId
WHERE schools.universityId = ?`, [universityId]);
    }
  ...
  router.post('/create/program', async (req: Request, res: Response) => {
    ...
    try {
      var yearNormalSqlResult: NormalPacket;
      var checkSubjectIdSqlResult: SelectPacket;
      var programNormalSqlResutl: NormalPacket = await dbConnection.executeInTransaction<NormalPacket>(transaction, 'INSERT INTO programs (programType, programName, programShortName, schoolId) VALUES (?, ?, ?, ?)', [programType, programName, programShortName, schoolId])
      for (var i = 0; i < years.length; i++) {
  ...
  router.get('/get/subject/:subjectId', async (req: Request, res: Response) => {
    ...
    var userId: number = await loginMethodsInterfaceObj.getLoggedInUserId();
    var universityId: number = await accountMethodsObj.getUserUniveristy(userId);
    var subjectSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>('SELECT subjectName, credits, archived FROM subjects WHERE subjectId = ? AND universityId = ?', [subjectId, universityId]);
    var responseObject: CustomResponseObject;
