...
export class AccountMethods {
    ...
    async getUserUniveristyLongId(userId: number): Promise<string> {
        var sqlResult: SelectPacket = await this.dbConnection.execute<SelectPacket>(`SELECT universities.universityLongId FROM universities
...
export function handleCheckDisabledMiddleware(dbConnection: DbHandler, loginMethodsObj: LoginMethods) {
    return async (req: Request, res: Response, next: NextFunction) => {
        ...
        var disabledSqlResult: SelectPacket = await dbConnection.execute<SelectPacket>('SELECT disabled FROM users WHERE userId = ?', [userId]);
        ...
        if (disabledSqlResult[0].disabled) {
            responseObject = {
                succ: false,
                disabled: true,
            };
...
export function handleCheckAdminMiddleware(dbConnection: DbHandler, loginMethodsObj: LoginMethods, accountMethodsObj: AccountMethods) {
    return async (req: Request, res: Response, next: NextFunction) => {
        var responseObject: CustomResponseObject;
        var sessionInterfaceObj: SessionInterface = req['sessionInterfaceObj'];
        var loginMethodsInterfaceObj: LoginMethodsInterface = new LoginMethodsInterface(sessionInterfaceObj, loginMethodsObj);
...
export function handleCheckProfessorMiddleware(dbConnection: DbHandler, loginMethodsObj: LoginMethods, accountMethodsObj: AccountMethods) {
    return async (req: Request, res: Response, next: NextFunction) => {
        var responseObject: CustomResponseObject;
        var sessionInterfaceObj: SessionInterface = req['sessionInterfaceObj'];
        var loginMethodsInterfaceObj: LoginMethodsInterface = new LoginMethodsInterface(sessionInterfaceObj, loginMethodsObj);
